Public Class receivableSale
    ...
    Private Sub itemName_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles filter.SelectedIndexChanged
        ...
        If filter.Text = "Custom Dates" Then
            ...
            fetch_data_UG("select ID, cName, tDate, phoneNumber, tType, bAmt from saleTable where tType = 'Credit' and tDate >= #" & dateFrom & "# and tDate <= #" & dateTo & "#")
        ...
        ElseIf filter.Text = "Today" Then
            ...
            fetch_data_UG("select ID, cName, tDate, phoneNumber, tType, bAmt from saleTable where tType = 'Credit' and tDate = #" & dateT & "#")
            fromDate.Enabled = False
            toDate.Enabled = False
            customerName.Enabled = False
        ...
        ElseIf filter.Text = "Past 1 Year" Then
            ...
            customerName.Enabled = False
        Else
            fetch_data_UG("select ID, cName, tDate, phoneNumber, tType, bAmt from saleTable where tType = 'Credit'")
            fromDate.Enabled = False
            toDate.Enabled = False
            customerName.Enabled = False
        End If
    ...
    Private Sub receivableSale_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Private Sub customerName_SelectedIndexChanged(sender As Object, e As EventArgs) Handles customerName.SelectedIndexChanged
        fetch_data_UG("select ID, cName, tDate, phoneNumber, tType, bAmt from saleTable where tType = 'Credit' and cName = '" & customerName.Text & "'")
    End Sub

    Private Sub fromDate_ValueChanged(sender As Object, e As EventArgs) Handles fromDate.ValueChanged
        Dim dateFrom As Date = CDate(Format(toDate.Value, "Short Date"))
        Dim dateTo As Date = CDate(Format(fromDate.Value, "Short Date"))
        fetch_data_UG("select ID, cName, tDate, phoneNumber, tType, bAmt from saleTable where tType = 'Credit' and pDate >= #" & dateFrom & "# and pDate <= #" & dateTo & "#")
    End Sub
    ...
    Private Sub DataGridView1_RowHeaderMouseDoubleClick(sender As Object, e As DataGridViewCellMouseEventArgs) Handles DataGridView1.RowHeaderMouseDoubleClick
        Dim recSalId = DataGridView1.Rows(e.RowIndex).Cells(0).Value
        If MessageBox.Show("Payment Received for this Transactio?", "StoreWise", MessageBoxButtons.YesNo, MessageBoxIcon.Information) = DialogResult.Yes Then
            ...
            Try
                ...
                Dim sql As String = "Update saleTable set tType = 'Cash' where ID = " & recSalId

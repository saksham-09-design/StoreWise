Public Class deleteSales
    ...
    Private Sub cancle_Click(ByVal sender As System.Object, ByVal e As System.EventArgs)
        Me.Close()
    End Sub

    Private Sub deleteSales_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Private Sub Delete_Click(sender As Object, e As EventArgs) Handles Delete.Click
        'adding return items to database
        Dim conn As New OleDb.OleDbConnection
        conn.ConnectionString = "Provider=Microsoft.ACE.OLEDB.12.0;Data Source=C:\Users\Saksham\Documents\StoreWise.accdb"
        Try
            If conn.State = ConnectionState.Closed Then
            ...
            Dim itemNo As Integer = itemList.Items.Count
            For i = 0 To itemNo - 1
                Dim sql As String = "Update inventoryTable set quantity = quantity + " & CInt(quantityList.Items(i)) & " where itemName = '" & itemList.Items(i).ToString & "'"
                Dim cmd As New OleDb.OleDbCommand(sql, conn)
                cmd.ExecuteNonQuery()
            Next
        Catch ex As Exception
        ...
        'deleting from sSuportTable
        Try
            If conn.State = ConnectionState.Closed Then
                conn.Open()
            End If
            Dim sql As String = "Delete * from sSupportTable where billId = " & salId
            Dim cmd As New OleDb.OleDbCommand(sql, conn)
            cmd.ExecuteNonQuery()
        Catch ex As Exception
            MessageBox.Show("Error: " & ex.Message, "Store Wise", MessageBoxButtons.OK, MessageBoxIcon.Error)
        ...
        'deleting from saleTable
        Try
            If conn.State = ConnectionState.Closed Then
                conn.Open()
            End If
            Dim sql As String = "Delete from saleTable where Id = " & salId
            Dim cmd As New OleDb.OleDbCommand(sql, conn)
            cmd.ExecuteNonQuery()
            MessageBox.Show("Transaction Deleted Successfully", "Storewise", MessageBoxButtons.OK, MessageBoxIcon.Information)
